...
public class OllirVisitor extends AJmmVisitor<Boolean, String> {
    ...
    public OllirVisitor(SymbolTable symbolTable) {
        ...
        addVisit("SUB_IMPORT", this::dealWithSubImport);
    ...
    private String dealWithImport(JmmNode jmmNode, Boolean tempVariable) {
        StringBuilder result = new StringBuilder("import " + jmmNode.get("name"));
        for (JmmNode child: jmmNode.getChildren())
            result.append(visit(child));
        return result + ";\n";
    }

    private String dealWithSubImport(JmmNode jmmNode, Boolean tempVariable) {
    ...
    private String dealWithIdentifier(JmmNode jmmNode, Boolean tempVariable) {
        ...
        if (symbolTable.isGlobal(var.getName()))
            ...
            before += "$" + (symbolTable.getMethod(ancestor.get().get("name")).getParamIndex(var.getName())+1) + ".";
        }
    ...
    private String escapeVarName(String varName) {
        if (varName.charAt(0) == '$')
            ...
            return "te_"+varName.substring(1);
        return varName;
    }
